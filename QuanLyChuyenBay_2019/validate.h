...
void CheckMoveAndValidateName_1(string &result, bool &isMove, int &ordinal, bool &isSave, int distance, int condition)
{
...
void CheckMoveAndValidateID(string &result, bool &isMove, int &ordinal, bool &isSave, int distance)
{
	...
	while (true)
	{
		while (_kbhit())
		{
			...
			if (key != 224 && key != SPACE && key != 0)
			{
				if ((key >= 65 && key <= 90) || (key >= 97 && key <= 122)  || (key >= 48 && key <= 57)) // 65-98 chu hoa; 97-122 chu thuong;48-57 chu so
				{
					if (count < MAX_LENGTH)
					{
						count++;
...
void CheckMoveAndValidateDateTime(int &result, bool &isMove, int &ordinal, bool &isSave, int condition, int distance)
{
	...
	Gotoxy(X_ADD + distance + ordinal * 3, 3 * 2 + Y_ADD);
	if (result !=0)
	cout << result;
	int count = lengh;
	bool isSpaced = false;
